...
The ownership decision below still holds.
What changed is the bare-command surface: `python` and `python3` now resolve to a uv-managed interpreter instead of refusing.
`pip` and `pip3` still refuse, as described here.
